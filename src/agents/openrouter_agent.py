...
openrouter_agent.py
-------------------
OpenRouter AI agent for VAIBHAV GROWTH ENGINE.

Uses the OpenAI-compatible ``openai`` SDK pointed at OpenRouter's API base
URL.  The default model is ``mistralai/mistral-7b-instruct`` but any model
available on https://openrouter.ai/models can be used by changing
``self.model``.  Returns ``None`` gracefully on any error so the
...
class OpenRouterAgent:
    """
    AI email and company-analysis agent backed by OpenRouter.

    OpenRouter exposes an OpenAI-compatible REST API, so this agent uses the
    ``openai`` SDK with a custom ``base_url``.  The client is only initialised
    when a valid API key is present.  All public methods catch exceptions and
    return ``None`` / empty dicts so that callers can implement seamless
    fallback logic.
    """

    def __init__(self) -> None:
        self.api_key: str = settings.OPENROUTER_API_KEY
        # Free model available on OpenRouter free tier
        self.model: str = "google/gemma-4-31b-it:free"
        self.base_url: str = "https://openrouter.ai/api/v1/chat/completions"
        self.headers: dict = {
            "Authorization": f"Bearer {self.api_key}",
            "Content-Type": "application/json",
            "HTTP-Referer": "https://github.com/Vaibhavsonava2005",
            "X-Title": "Vaibhav Growth Engine",
        }
        self._configured: bool = self.is_configured()
        logger.debug(
            "OpenRouterAgent initialised | model={} | configured={}",
            self.model,
            self._configured,
        )
    ...
    def generate_email(
        ...
        """
        Generate a personalised cold email via OpenRouter / Mistral 7B.
        ...
        if not self.is_configured():
            logger.debug("OpenRouterAgent: not configured, skipping")
            return None
        ...
        try:
            logger.debug(
                "OpenRouterAgent: calling {} for {}", self.model, contact.full_name
            )

            raw_text: str = self._call_api(
                system="You are an expert B2B sales development representative. "
            ...
            if not parsed.get("subject") or not parsed.get("body"):
                logger.warning(
                    "OpenRouterAgent: could not parse structured response for {}",
                    contact.full_name,
                )
                return None

            # Follow-up 1 (3 days)
            fu1_text = self._call_api(
                system="You are a B2B SDR writing concise follow-up emails.",
                user=follow_up_prompt_3d,
                max_tokens=512,
                temperature=0.7,
            )
            follow_up_1 = self._extract_body(fu1_text or "")

            # Follow-up 2 (7 days)
            fu2_text = self._call_api(
                system="You are a B2B SDR writing concise follow-up emails.",
                user=follow_up_prompt_7d,
                max_tokens=512,
                temperature=0.7,
            )
            follow_up_2 = self._extract_body(fu2_text or "")
            ...
            logger.info(
                "OpenRouterAgent: email generated for {} (score={:.1f})",
                contact.full_name,
                score,
            )
            return result
            ...
            logger.error(
                "OpenRouterAgent: generate_email failed for {} — {}",
                contact.full_name,
                exc,
            )
            return None
    ...
    def analyze_company(
        ...
        """
        Extract structured intelligence from scraped website content via OpenRouter.
        ...
        if not self.is_configured():
            logger.debug("OpenRouterAgent: not configured for company analysis")
            return empty
        ...
        try:
            logger.debug("OpenRouterAgent: analysing company '{}'", company_name)

            raw_text = self._call_api(
                system="You are a business analyst extracting structured sales "
            ...
            logger.info(
                "OpenRouterAgent: analysis complete for '{}' — {} pain points, {} opportunities",
                company_name,
            ...
            logger.error(
                "OpenRouterAgent: analyze_company failed for '{}' — {}",
                company_name,
                exc,
            )
            return empty
    ...
    def _call_api(
        ...
        """Make a direct httpx POST to OpenRouter chat/completions. Returns content string or None."""
        payload = {
            "model": self.model,
            "messages": [
        ...
        try:
            ...
            if resp.status_code == 402:
                logger.warning("OpenRouter credits exhausted (402)")
                return None
            ...
            logger.error("OpenRouter _call_api failed: {}", exc)
            return None
